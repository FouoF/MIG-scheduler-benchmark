...
主碎片实验必须使用预热填充：同构的 2 节点 × 4 H100 集群先提交 56 个长时间
`1g.10gb` 等价资源请求，覆盖全部 56 个 GPC placement；随后才进入统计窗口。
后续到达率设置为高于估算服务率，持续维持待调度队列。默认任务计算时间中位数为
600 秒，显著高于毫秒级调度延迟。

统计窗口从 `measurementStartMS` 开始，到最后一个 workload 到达为止，排除预热
爬坡和最终 drain。每次 run 必须同时满足：

- 峰值 GPC 利用率不低于 99%；
- 测量窗口队列非空时间占比不低于 90%；
- 实际默认验收目标为两个值分别达到 100% 和至少 99%。

未达到阈值时设置 `highLoadValid=false`；启用 `requireHighLoad` 后整次 run 失败，
不得进入跨后端聚合。测量期平均利用率不强制达到 100%，因为饱和需求下仍未使用的
容量正是碎片化或合法 placement 限制所造成的结果。
